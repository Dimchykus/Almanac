...
- **Icons**: Use `@tabler/icons-react` directly (e.g. `import { IconLink } from "@tabler/icons-react"`). The legacy `AlmIcon` component (`src/components/alm-icon/index.tsx`) still exists for existing usages but new icons should come from Tabler.
